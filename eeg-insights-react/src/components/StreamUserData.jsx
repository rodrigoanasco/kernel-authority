...
async function bulkInsertUserData(rows, setProgress, consentId) {
  ...
  for (let i = 0; i < rows.length; i += chunk) {
    const slice = rows.slice(i, i + chunk).map(r => ({
      ...
      consent_id: consentId ?? null,   // <-- link rows to consent session
      chan: r.chan,
      seconds: r.seconds,
      voltage: r.voltage
    }))
...
export default function StreamUserData({ consentId }) {
  ...
  async function onFile(e) {
    try {
      ...
      await bulkInsertUserData(rows, setProgress, consentId)
      setStatus('Done ✅')
    } catch (err) {
